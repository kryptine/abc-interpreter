...
UI *code;
int allocated_code_size,code_size;
UI *data;
int allocated_data_size,data_size;

struct relocation *code_relocations;
int n_code_relocations,n_allocated_code_relocations;

struct relocation *data_relocations;
int n_data_relocations,n_allocated_data_relocations;
...
//	allocated_code_size=8000; /* words */
	allocated_code_size=1000; /* words */
//	n_allocated_code_relocations=1400;
	n_allocated_code_relocations=500;
//	allocated_data_size=1100; /* words */
	allocated_data_size=1000; /* words */
//	n_allocated_data_relocations=300;
	n_allocated_data_relocations=500;

	code_size=0;
	code=malloc (allocated_code_size*sizeof (UI));
	if (code==NULL){
		printf ("Allocating memory for code failed\n");
		exit (1);
	}

	data_size=0;
	data=malloc (allocated_data_size*sizeof (UI));
	if (data==NULL){
		printf ("Allocating memory for data failed\n");
		exit (1);
	}

	n_code_relocations=0;
	code_relocations=malloc (n_allocated_code_relocations*sizeof (struct relocation));
	if (code_relocations==NULL){
		printf ("Allocating memory for code relocations failed\n");
		exit (1);
	}

	n_data_relocations=0;
	data_relocations=malloc (n_allocated_data_relocations*sizeof (struct relocation));
	if (data_relocations==NULL){
		printf ("Allocating memory for data relocations failed\n");
		exit (1);
	}
}
...
{
	int new_allocated_code_size;
	
	new_allocated_code_size=allocated_code_size+((allocated_code_size>>1)&-4);
	code=realloc (code,new_allocated_code_size*sizeof (UI));
	if (code==NULL){
		printf ("Reallocating memory for instructions failed (%d bytes)\n",new_allocated_code_size*sizeof (UI));
		exit (1);
	}
	allocated_code_size=new_allocated_code_size;
}

void store_code_w (UI w)
{
	if (code_size>=allocated_code_size)
		realloc_code();
...
struct label *new_internal_label (void)
{
	...
	new_label_p=fast_memory_allocate_type (struct label);
...
struct label *enter_label (char *label_name)
{
	...
	new_label_node_p=fast_memory_allocate_type (struct label_node);
	new_label_p=fast_memory_allocate_type (struct label);
	...
	new_label_p->label_name=(char*)fast_memory_allocate (strlen (label_name)+1);
...
void store_data_l (UI v)
{
	if (data_size>=allocated_data_size){
		...
		data=realloc (data,new_allocated_data_size*sizeof (UI));
		if (data==NULL){
			printf ("Reallocating memory for data failed (%d bytes)\n",new_allocated_data_size*sizeof (UI));
		exit (1);	
	}
...
void store_code_internal_label_value (struct label *label,int offset)
{
...

	{
		...
		relocation_p->relocation_offset=code_size;
...
static struct label Fadd_arg_labels[N_ADD_ARG_LABELS]
	= {
		{/*label_name*/ "_add_arg0", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg1", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg2", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg3", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg4", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg5", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg6", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg7", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg8", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg9", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg10", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg11", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg12", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg13", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg14", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg15", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg16", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg17", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg18", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg19", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg20", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg21", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg22", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg23", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg24", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg25", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg26", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg27", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg28", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg29", /*label_offset*/ -1 },
		{/*label_name*/ "_add_arg30", /*label_offset*/ -1 },
...
UI *relocate_code_and_data (int add_code_or_data_offset)
{
	...
	for (i=0; i<N_ADD_ARG_LABELS; ++i)
		if (Fadd_arg_label_used[i]){
			Fadd_arg_labels[i].label_offset = code_size<<2;
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %s\n",code_size,instruction_name (i),label_name);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %s%+d\n",code_size,instruction_name (i),label_name,offset);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %s%+d %s\n",code_size,instruction_name (i),label_name1,offset,label_name2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %s%+d %d\n",code_size,instruction_name (i),label_name,offset,(int)n1);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %s%+d %d %s\n",code_size,instruction_name (i),label_name1,offset,(int)n1,label_name2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %s%+d %s %s%+d %s\n",code_size,instruction_name (i),label_name1,offset1,label_name2,
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %s %d %d\n",code_size,instruction_name (i),label_name,(int)n1,(int)n2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d\n",code_size,instruction_name (i),(int)n);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d\n",code_size,instruction_name (i),(int)n1,n2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s\n",code_size,instruction_name (i),(int)n,label_name);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s%+d\n",code_size,instruction_name (i),(int)n,label_name,offset);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s%+d %s\n",code_size,instruction_name (i),(int)n,label_name1,offset,label_name2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s %d\n",code_size,instruction_name (i),(int)n1,label_name,(int)n2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s%+d %s %s%+d %s\n",code_size,instruction_name (i),n1,label_name1,offset1,label_name2,
																				  label_name3,offset2,label_name4);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s+%d %d\n",code_size,instruction_name (i),(int)n1,label_name,offset,(int)n2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s+%d %d %d\n",code_size,instruction_name (i),(int)n1,label_name,offset,(int)n2,(int)n3);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %s+%d %d %d %d\n",code_size,instruction_name (i),(int)n1,label_name,offset,(int)n2,(int)n3,(int)n4);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s\n",code_size,instruction_name (i),(int)n1,(int)n2,label_name);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s%+d\n",code_size,instruction_name (i),(int)n1,(int)n2,label_name,offset);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s %d\n",code_size,instruction_name (i),(int)n1,(int)n2,label_name,(int)n3);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s %d %s\n",code_size,instruction_name (i),(int)n1,(int)n2,label_name1,(int)n3,label_name2);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s+%d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,label_name,offset,(int)n3);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,label_name,(int)n3,(int)n4);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s+%d %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,label_name,offset,(int)n3,(int)n4);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d %s\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3,label_name);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d %s+%d\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3,label_name,offset);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d %s+%d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3,label_name,offset,(int)n4);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d %s %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3,label_name,(int)n4,(int)n5);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d %s+%d %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3,label_name,offset,(int)n4,(int)n5);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3,(int)n4);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %d %d %d\n",code_size,instruction_name (i),(int)n1,(int)n2,(int)n3,(int)n4,(int)n5);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d\n",code_size,instruction_name (i),n);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d\n",code_size,instruction_name (i),label->label_offset);
...
{
	if (list_code || i>max_implemented_instruction_n)
		printf ("%d\t%s %d %d %s\n",code_size,instruction_name (i),n1,label->label_offset,label_name);
...
{
	add_instruction_w_w_label (Cjmp_neI_b,b_offset,value,label_name);
}
...
void code_label (char *label_name)
{
	...
	label->label_offset=code_size<<2;
}
...
void code_n (int number_of_arguments,char *descriptor_name,char *ea_label_name)
{
	...
	add_data4_to_code (number_of_arguments);
}
...
void code_nu (int a_size,int b_size,char *descriptor_name,char *ea_label_name)
{
	...
	add_data4_to_code (a_size+b_size+(b_size<<8));
}

void code_o (int oa,int ob,ULONG vector[])
{
	if (last_jsr_with_d){
		if (code[code_size-2]!=Cjsr){
			int i;
			
			i=code[code_size-3];
			if (i!=Cpop_a_jsr && i!=Cpop_b_jsr && i!=Cpush_a_jsr && i!=Cpush_b_jsr
				&& code[code_size-4]!=Cbuildh0_put_a_jsr)
			{
...
#if 0
		code[code_size-2]=Cjsr_stack_check;
		if (list_code)
			printf ("%d\t%d %d\n",code_size,oa-last_da,ob-last_db);
		store_code_w (oa-last_da);
		store_code_w (ob-last_db);
...
static void print_code_or_data (int segment_size,UI *segment,int comma_separator,FILE *program_file)
{
	char *f1,*f2;
...
void write_program (void)
{
	...
	fprintf (program_file,"%d %d %d\n",code_size,n_code_code_relocations,n_code_data_relocations);
	fprintf (program_file,"%d %d %d\n",data_size,n_data_code_relocations,n_data_data_relocations);

	print_code_or_data (code_size,code,0,program_file);
...
void write_js_program (void)
{
	...
	program_file = fopen ("program.js","w");
	if (program_file==NULL){
		printf ("Could not create program file\n");
		exit (1);
	}
	...
	fprintf (program_file,"var code = [\n");
	print_code_or_data (code_size,code,1,program_file);
	fprintf (program_file,"];\n");
	fprintf (program_file,"var code_code_relocations = [\n");
	print_code_or_data_code_relocations (n_code_code_relocations,n_code_relocations,code_relocations,1,program_file);
	fprintf (program_file,",];\n");
	fprintf (program_file,"var code_data_relocations = [\n");
	print_code_or_data_data_relocations (n_code_data_relocations,n_code_relocations,code_relocations,1,program_file);
	fprintf (program_file,",];\n");

	fprintf (program_file,"var data = [\n");
	print_code_or_data (data_size,data,1,program_file);
	fprintf (program_file,"];\n");
	fprintf (program_file,"var data_code_relocations = [\n");
	print_code_or_data_code_relocations (n_data_code_relocations,n_data_relocations,data_relocations,1,program_file);
	fprintf (program_file,"];\n");
	fprintf (program_file,"var data_data_relocations = [\n");
	print_code_or_data_data_relocations (n_data_data_relocations,n_data_relocations,data_relocations,1,program_file);
	fprintf (program_file,"];\n");
