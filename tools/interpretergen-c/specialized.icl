...
instr_RtoAC_32 t = foldl (flip append) t
	...
	, "((uint32_t*)&dr)[1]=bsp[1];"
	, "#endif"
	, "int n=sprintf(r,BC_REAL_FMT,dr+0.0);"
	, "NEED_HEAP(2+((n+IF_INT_64_OR_32(7,3))>>IF_INT_64_OR_32(3,2)));"
	, "hp[0]=(BC_WORD)&__STRING__+2;"
	, "hp[1]=n;"
	, "memcpy(&hp[2],r,n);"
...
instr_readFR_32 t = foldl (flip append) t
	...
	, "} else if (F_IS_TEXT_MODE(f->file_mode)) {"
	, "#if WORD_WIDTH==32"
	, "\tbsp[0]=fscanf(f->file_handle,BC_DREAL_SCAN_FMT,(BC_DREAL*)&bsp[1])==1;"
	, "#else"
	, "\tBC_DREAL _temp_dreal;"
	, "\tbsp[0]=fscanf(f->file_handle,BC_DREAL_SCAN_FMT,&_temp_dreal)==1;"
	, "\tbsp[1]=((uint32_t*)&_temp_dreal)[0];"
	, "\tbsp[2]=((uint32_t*)&_temp_dreal)[1];"
	, "#endif"
	, "} else {"
	, "\tint i;"
	, "\tbsp[0]=1;"
